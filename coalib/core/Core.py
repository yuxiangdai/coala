...
def initialize_dependencies(bears):
    """
    Initializes and returns a ``DependencyTracker`` instance together with
    a set of bears ready for scheduling.

    This function acquires, processes and registers bear dependencies
    accordingly using a consumer-based system, where each dependency bear
    has only a single instance per section and file-dictionary.

    The bears set returned accounts for bears that have dependencies and
    excludes them accordingly. Dependency bears that have themselves no
    further dependencies are included so the dependency chain can be
    processed correctly.
    ...
    # For a consumer-based system, we have a situation which can be
    ...
    grouping = group(bears,
                     key=lambda bear: (bear.section, bear.file_dict))
    for (section, file_dict), bears_per_section in grouping:
        ...
        # Now traverse each edge of the graph, and instantiate a new
        # dependency bear if not already instantiated. For the entry point
        # bears, we hack in identity-mappings because those are already
        # instances. Also map the types of the instantiated bears to those
        # instances, as if the user already supplied an instance of a
        # dependency, we reuse it accordingly.
    ...
    # Get all bears that aren't resolved and exclude those from scheduler
    # set.
...
class coalaRunSession:
    def __init__(self, bears, result_callback):
        self.result_callback = result_callback

        self.running_tasks = {}
        self.dependency_tracker, self.initial_bears_to_schedule = (
            initialize_dependencies(bears))
    ...
    def cleanup_bear(self, bear):
        ...
        - Checks whether there are any remaining tasks, and quits the event
          loop accordingly if none are left.
        ...
        if not self.running_tasks[bear]:
            ...
            if resolved_bears:
                self.schedule_bears(resolved_bears)
    ...
    def schedule_bears(self, bears):
        ...
        for bear in bears:
            ...
            if self.dependency_tracker.get_dependencies(bear):
                ...
                tasks = {
                    self.event_loop.run_in_executor(self.executor,
                                                    bear.execute_task,
                                                    bear_args,
                                                    bear_kwargs)
                ...
                for task in tasks:
                    task.add_done_callback(
                        functools.partial(self.finish_task, bear))
                ...
                if not tasks:
                    ...
                    self.cleanup_bear(bear)

    def finish_task(self, bear, task):
        """
        ...
        try:
            ...
            logging.debug('Following dependent bears were unscheduled: ' +
                          ', '.join(
                              repr(dependant) for dependant in dependants))
        finally:
    ...
    def run(self):
        ...
        """
        self.schedule_bears(self.initial_bears_to_schedule)
...
def run(bears, result_callback):
    ...
    session = coalaRunSession(bears, result_callback)
